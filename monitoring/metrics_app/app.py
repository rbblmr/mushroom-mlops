...
def fetch_data():
    ...
    data = client.get_database("mushroom-web-service").get_collection("data").find()
...
def run_evidently(ref_data, data):
    ...
    mapping = ColumnMapping(prediction="class", numerical_features=['trip_distance'],
...
def save_html_report(result, result_path):
    result[1].save(result_path)

def load_html_report(result_path):
    with open(result_path, 'r', encoding='utf-8') as file:
        dashboard=file.read()
    return dashboard

def batch_analyze():
    result_path="./dashboards/monitoring_dashboard.html"
    ref_data = pd.read_csv("./datasets/mushroom/reference.csv")
    data = fetch_data()
    result = run_evidently(ref_data, data)
    save_report(result)
    save_html_report(result, result_path)
    
    return load_html_report(result_path)
